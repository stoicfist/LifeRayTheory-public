...
\fancyhead[R]{\textcolor{gray}{\small\textit{Part II -- Formal Framework}}}

% --- Footer ---
\fancyfoot[L]{\textcolor{gray}{\footnotesize Independent Conceptual Research}}

% clickable DOI link:
\fancyfoot[R]{\footnotesize\href{https://doi.org/10.5281/zenodo.19145600}{\textcolor{gray!65!black}{DOI:10.5281/zenodo.19145600}}}
...
\vspace{1em}
\begin{center}
\textcolor{gray}{\small \textit{Preprint – Work in progress – Part II of a four-part research project.}}
\end{center}
\vspace{1em}
...
coupling, and dynamical evolution will rely. In this way, the present text serves as the
bridge between the intuitive origins of the theory and its scientific articulation.

At this stage, however, the formal structures introduced here are intentionally defined at
an abstract and phenomenological level. They are not meant to directly represent neural
mechanisms or physiological processes. Rather, they provide a symbolic and structural
scaffold that may later be mapped onto cognitive and neurobiological models of memory,
salience, and affective processing.

Accordingly, the present formalization primarily addresses the reconstruction and
organization of experience rather than its biological generation. A mechanistic grounding
in terms of neural, bodily, and regulatory processes is acknowledged as essential, but is
explicitly deferred to later stages of the project.

Future developments will therefore explore possible correspondences between the abstract
objects defined here (such as fragments, life rays, and integration operators) and
established constructs in cognitive neuroscience, including memory consolidation,
affective tagging, and attentional dynamics.
...
Each agent $k \in K$ is associated with an own space $S_k$~\index{Core Concepts!own space@\textit{own space} \ensuremath{S_k}}, representing its internal experiential state space.

\subsection{Life Rays}

A life ray $\gamma_k$~\index{Core Concepts!life ray@\textit{life ray} \ensuremath{\gamma_k}} is defined as a time-parametrized curve
\begin{equation}
...
An encounter between two agents $i,j \in K$ at time $t$ is modeled as an event~\index{Core Concepts!event@\textit{event} \ensuremath{E_{ij}(t)}}
\begin{equation}
E_{ij}(t).
\end{equation}
...
Fragments arise from the transduction of shared events into the internal space of an
agent. For each event $E_{ij}(t)$ and each involved agent $k$, this process is described
by the transduction operator~\index{Structural Operators!transduction@\textit{transduction} \ensuremath{T}}
\[
T_k : E_{ij}(t) \longrightarrow F_k .
\]

A fragment~\index{Core Concepts!fragment@\textit{fragment} \ensuremath{F_k}}
is minimally represented as
\begin{equation}
F_k = (t, v_k),
\end{equation}
where $t$ is the time of the event and $v_k$ is an imprint of the agent’s internal state
induced by the encounter.

\subsection{Fragment Chains}

Each agent $k$~\index{Core Concepts!agent@\textit{agent} \ensuremath{k}} is associated with a fragment chain
\index{Core Concepts!fragment chain@\textit{fragment chain} $\mathcal{F}_k(t)$},
which represents the ordered sequence of memory fragments at time $t$:
\begin{equation}
  \mathcal{F}_k(t) = (F_k^{(1)}, F_k^{(2)}, \dots, F_k^{(n)}).
  \label{eq:fragment-chain}
\end{equation}
The chain contains all fragments generated by agent $k$ up to time $t$.
This sequential growth of the fragment chain is illustrated schematically in Figure~\ref{fig:fragment-chain}

\begin{figure}[h]
\centering
\begin{tikzpicture}[
    node distance=3cm,
    every node/.style={circle, draw, minimum size=11mm, font=\small},
    arrow/.style={->, thick}
]
\node (F1) {$F_k^{(1)}$};
\node (F2) [right=of F1] {$F_k^{(2)}$};
\node (F3) [right=of F2] {$F_k^{(3)}$};
\node (F4) [right=of F3] {$F_k^{(n)}$};
...
% Ellipsis zwischen middle nodes
\node[draw=none] at ($(F2)!0.5!(F3)$) {$\dots$};

% Events above
\node[draw=none, above=2cm of F1] (E1) {$E_{ij}^{(1)}$};
\node[draw=none, above=2cm of F2] (E2) {$E_{ij}^{(2)}$};
\node[draw=none, above=2cm of F3] (E3) {$E_{ij}^{(3)}$};
\node[draw=none, above=2cm of F4] (E4) {$E_{ij}^{(n)}$};


\draw[arrow, dashed] (E1) -- (F1);
\draw[arrow, dashed] (E2) -- (F2);
\draw[arrow, dashed] (E3) -- (F3);
\draw[arrow, dashed] (E4) -- (F4);

% Time axis
\node[draw=none, below=1cm of F1] {$t \rightarrow$};
\end{tikzpicture}

\caption{Fragment chain $\mathcal{F}_k(t)$ as an ordered sequence
$(F_k^{(1)}, F_k^{(2)}, \dots, F_k^{(n)})$.
Encounter events generate fragments that are sequentially appended over time.}
\label{fig:fragment-chain}
\end{figure}
...
we now enrich events~\index{Extended Structures!extended event@\textit{extended event}\ensuremath{E_{ij}=(t,C,D,I,V,R)}} with descriptive and phenomenological parameters:
...
\begin{figure}[h]
    \centering
        \begin{tikzpicture}[x=1cm,y=1cm,>=stealth,thick]
            % Time axis
            \draw[->,gray!70] (0,0.2) -- (10,0.2) node[right]{Time};
            ...
            % Intersection events
            \filldraw[blue!70] (4.0,1.2) circle (2.2pt);
            \filldraw[red!70]  (4.0,-0.8) circle (2.2pt);
            \draw[->,gray!70] (4.0,0.55) -- (4.0,0.95);
            \draw[->,gray!70] (4.0,-0.1) -- (4.0,-0.55);
            \node at (4.0,0.25) {\footnotesize $E_{ij}^{(1)}$};

            \filldraw[blue!70] (5.0,1.2) circle (2.2pt);
            \filldraw[red!70]  (5.0,-0.8) circle (2.2pt);
            \draw[->,gray!70] (5.0,0.55) -- (5.0,0.95);
            \draw[->,gray!70] (5.0,-0.1) -- (5.0,-0.55);
            \node at (5.0,0.25) {\footnotesize $E_{ij}^{(2)}$};

            \filldraw[blue!70] (6.0,1.2) circle (2.2pt);
            \filldraw[red!70]  (6.0,-0.8) circle (2.2pt);
            \draw[->,gray!70] (6.0,0.55) -- (6.0,0.95);
            \draw[->,gray!70] (6.0,-0.1) -- (6.0,-0.55);
            \node at (6.0,0.25) {\footnotesize $E_{ij}^{(3)}$};
...
\]
where $s_k$ is the salience~\index{Semantic / Dynamical Quantities!salience@\textit{salience} \ensuremath{s_k}}, $\phi_k$ a semantic label,
...
We denote by $\mathbin{\|}$ the concatenation~\index{Structural Operators!concatenation@\textit{concatenation} $\vert\vert$}

 operator on fragment chains, such that
\begin{equation}
...
and salience $s$.
Subsequent \emph{alignment}~\index{Semantic / Dynamical Quantities!alignment@\textit{alignment} \ensuremath{A(F_i,F_j)}}
 through shared recall or narration
...
Operationally, a \emph{twist index}~\index{Semantic / Dynamical Quantities!twist index@\textit{twist index} \ensuremath{\mathrm{twist}_{ij}}} may be updated incrementally
after each event:
\[
\text{twist}_{ij}
\;\leftarrow\;
...
We model this process by an integration operator $A_k$~\index{Structural Operators!integration@\textit{integration} $A_k$} acting on the memory structure of a
single agent $k$:
...
As a first approximation, the intensity of humor\index{Semantic / Dynamical Quantities!humor@\textit{humor} \ensuremath{A_{\mathrm{humor}}}} may be expressed as
\begin{equation}
